...
async function ensureSchema(pool) {
  ...
  try {
    ...
    `);
    // Colunas adicionadas depois da primeira versão — ALTER separado porque
    // CREATE TABLE IF NOT EXISTS não altera uma tabela já existente em produção.
    await client.query(`ALTER TABLE ${SCHEMA}.orders ADD COLUMN IF NOT EXISTS discount NUMERIC NOT NULL DEFAULT 0`);
    await client.query(`ALTER TABLE ${SCHEMA}.orders ADD COLUMN IF NOT EXISTS channel TEXT NOT NULL DEFAULT 'loja'`);
    await client.query(`ALTER TABLE ${SCHEMA}.orders ADD COLUMN IF NOT EXISTS seller_name TEXT`);
    await client.query(`ALTER TABLE ${SCHEMA}.orders ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT now()`);
